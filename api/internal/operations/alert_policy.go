...
var alertPolicies = map[string]AlertPolicy{
	"node_offline":                    {AlertSpec: database.AlertSpec{Category: "connection", Title: "节点离线"}},
	...
	"ipp_unreachable":                 {AlertSpec: database.AlertSpec{Category: "connection", Title: "打印机无法连接"}, ActivationDelay: 60 * time.Second, ConnectionScope: true},
	"printer_offline":                 {AlertSpec: database.AlertSpec{Category: "connection", Title: "打印机离线"}, ConnectionScope: true},
	...
	"printer_state_unknown":           {AlertSpec: database.AlertSpec{Category: "device", Title: "无法确认打印机状态"}, ActivationDelay: 60 * time.Second, ConnectionScope: true},
}
...
func connectionScopedPrinterReasons() []string {
	reasons := []string{}
	for reason, policy := range alertPolicies {
		if policy.ConnectionScope {
