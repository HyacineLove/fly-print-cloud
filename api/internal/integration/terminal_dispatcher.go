...
func (d *TerminalDispatcher) ProcessOne() error {
	...
	if err != nil || request == nil { return err }
	...
	if err != nil || !matched || !d.manager.IsNodeConnected(request.NodeID) { return err }
	printer, err := d.printers.GetPrinterByID(request.PrinterID)
	if err != nil || printer == nil || !printer.Enabled || printer.EdgeNodeID != request.NodeID { return err }
	if request.FileID == nil { return nil }
	file, err := d.files.GetByID(*request.FileID)
	if err != nil || file == nil { return err }
	options := struct { Copies int `json:"copies"`; PaperSize string `json:"paper_size"`; ColorMode string `json:"color_mode"`; DuplexMode string `json:"duplex_mode"` }{Copies: 1}
	if err := json.Unmarshal(request.PrintOptions, &options); err != nil { return err }
	...
	if err := d.jobs.CreatePrintJob(job); err != nil { return err }
	if err := d.manager.DispatchPrintJob(request.NodeID, job, printer.Name); err != nil {
	...
	if err := d.jobs.MarkDispatched(job.ID); err != nil { return err }
